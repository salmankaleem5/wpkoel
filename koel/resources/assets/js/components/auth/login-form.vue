<template>
  <form @submit.prevent="login" :class="{ error: failed }">
    ...
    <input v-model="wpLoginCheck" id="wpLoginCheck" type="checkbox">
    <label for="wpLoginCheck">Log in with WordPress</label>
  </form>
</template>
...
export default {
  ...
  methods: {
    async login () {
      try {
        await userStore.login(this.email, this.password,this.wpLoginCheck)
        this.failed = false
