...
export const playback = {
  ...
  init () {
    ...
    player.addEventListener('timeupdate', e => {
      ...
      // Record duration every 5 seconds if play count has been registered
      if( song.registeredPlayCount && (this.player.media.currentTime - this.lastRecordedTime) >= 5 ){
        this.lastRecordedTime = this.player.media.currentTime

        songStore.saveListenTime(song.id, this.player.media.currentTime)
      }
    })
